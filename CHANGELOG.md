...
- **Hybrid search no longer enumerates the knowledge graph on every query.** `GraphRetrieval.searchByEntities` and `expandFromChunks` each began with a full `kv.list` of `KV.graphNodes` plus `KV.graphEdges`, and `tripleStreamSearch` called both — two to four unpaginated enumerations per search. On a 32,127-node / 61,577-edge graph that measured 60.8 MB of JSON and ~2.1s of KV time per query, and grew iii-engine RSS by 573 MB that was never released, so a service running `AGENTMEMORY_INJECT_CONTEXT=true` (a search per file-touching tool call) climbed until the container was OOM-killed. Every result was discarded anyway: retrieval hardcodes an empty `sessionId`, so hydration read scope `mem:obs:` and missed (0 of 20 graph hits hydrated in a direct probe; 20 of 20 with the correct session id). The stream now sits behind `AGENTMEMORY_GRAPH_SEARCH`, off by default, with the missing session id resolved from the BM25 index so enabling it is meaningful. When enabled it is bounded: adjacency is built once per call instead of once per seed (an uncapped call measured 62s), seeds are capped by `AGENTMEMORY_GRAPH_SEARCH_MAX_SEEDS`, and traversal has a wall-clock budget. Full evidence in `docs/investigations/2026-08-24-latency-and-oom.md`.
- **Graph extraction advances its watermark on an empty batch.** Heuristic extraction only builds nodes from `obs.files` and `obs.concepts`, and synthetic compression leaves concepts empty, so a zero-result extraction is the common case. The zero-result early return sat above the watermark stamp, so the cursor never moved and every session stop re-listed and re-extracted the same growing batch — which is what inflated the graph the search path then had to scan. An LLM failure still leaves the cursor alone so the batch is retried.
- **`/observe` no longer waits on the embedding provider inside the session lock.** Ingest awaited `vectorIndexAddGuarded`, a provider round-trip, inside `withKeyedLock("obs:" + sessionId)`, so one slow embedding serialised every later observe for that session while both sat on a 2500 ms hook budget. The embedding moves to a queue with bounded concurrency and a bounded backlog; the vector index is derived and rebuilt from KV at boot. Measured p50 1.05s → 0.05s under a concurrent hook burst.

### Changed

- **Graph search uses write-path indexes instead of the snapshot tables.** `GraphRetrieval.searchByEntities` and `expandFromChunks` looked up names and neighbors by scanning the snapshot (top-N only, and still a full in-memory walk). They now use token, adjacency, and observation indexes written by `persistGraphDelta`, temporal extract, mesh apply/receive, and cascade stale flags. Keys are namespaced by `resetAt` so pre-reset orphan rows are never read. First search backfills from the snapshot once per reset; it does not `kv.list` `KV.graphNodes` / `KV.graphEdges`. `AGENTMEMORY_GRAPH_SEARCH` stays off by default.

- **Graph readers no longer list `KV.graphNodes` / `KV.graphEdges`.** After a graph reset those scopes still hold orphaned pre-reset rows; a live `kv.list` is the old OOM path. `mem::graph-query` (including `query` / `startNodeId`), `GraphRetrieval`, temporal query and extract merge, MCP `agentmemory://graph/stats`, export, `mem::reflect`, mesh graph sync, `mem::snapshot-create`, and `mem::cascade-update` now read the snapshot only. Cascade, temporal extract, and mesh apply write the snapshot back so stale flags and merges stay on the live graph. Snapshot upserts still count past the top-N cap and do not increment the same id twice. Extract merge does not re-count stale rows. Mesh skips pre-`resetAt` rows so a peer cannot resurrect orphans. Cascade flags edges before nodes so edge totals drop. `mem::graph-query` warns when `stats.totalNodes` exceeds the snapshot's top-N. `mem::graph-snapshot-rebuild` refuses after `resetAt` even with `force`, so it cannot resurrect the junk. Incremental extract still fills the snapshot.

- **Periodic consolidation skips the LLM pass when summaries are unchanged.** `mem::consolidate-pipeline` hashed nothing, so every two-hour tick re-extracted facts and insights from the same corpus (production: 89 summaries, 7–16 `newFacts` and exactly 5 `newInsights` per idle run because exact-string / content-hash dedup lost to LLM wording). It now stores a SHA-256 of the 20 summaries sent to the merge prompt on `KV.config` and skips semantic extraction plus `mem::reflect` when that payload is identical. A semantic-only run stamps the hash without `reflected`, so a later `tier: "all"` still generates insights. A failed semantic pass does not stamp, even if reflect succeeds. `force: true` still bypasses the enabled check only, not this skip. Decay still lists the tables, but writes only rows whose `strength` actually changed, stamps `updatedAt` so a later tick in the same idle window is a no-op, and reports `{ scanned, written }` instead of the table length.
- **Background LLM jobs now share a concurrency gate.** `mem::compress` (4.5s avg), `mem::summarize` (6.9s avg), and the graph-extract LLM pass were fire-and-forget from observe / session stop / idle sweep, so a hook burst started one provider call per event on the same event loop. They now queue behind `AGENTMEMORY_BACKGROUND_LLM_CONCURRENCY` (default 2). `/health` reports `backgroundLlmInFlight` / `backgroundLlmQueued`. Structural graph extraction still runs ungated.

- **Bounded work on the hook endpoints.** `iii`'s HTTP trigger exposes no abort signal and the SDK protocol has no cancel message, so a client that gives up at 2500 ms leaves the server working: a request aborted at 0.5s was measured still acquiring 212 MB of engine RSS afterwards. `/enrich` now runs under `AGENTMEMORY_ENRICH_BUDGET_MS` (default 1200) and returns the branches that finished; `mem::file-context` stops its per-session scan at the same budget instead of loading every observation of the 15 most recent sessions (up to 30,000 at `MAX_OBS_PER_SESSION=2000`); concurrent searches are capped by `AGENTMEMORY_SEARCH_CONCURRENCY` (default 4) so a burst queues instead of multiplying peak memory; and state reads have a ceiling via `AGENTMEMORY_KV_TIMEOUT_MS` (default 15s) rather than inheriting the 180s iii invocation timeout. Writes are deliberately left unbounded: a timeout is not a cancellation, so a write that rejects client-side can still commit, and `mem::observe`'s rollback would race that late commit and orphan the row it exists to clean up.
- **`mem::file-context` reuses its enumerations.** It ran one `kv.list` of `KV.sessions` plus one per candidate session on every file-touching tool call — 16 enumerations per enrich against 1,064 sessions in one deployment. A short-TTL cache holds the session list and each session's candidate observations projected down to the fields the file matcher uses, bounded by a global entry budget; `/observe` drops its own session's entry. Session enumerations fell to 3 per 240 enrich calls, per-session observation enumerations from 15 per enrich to 2.5.
- **Health reports container memory and load, not just heap ratio.** The memory alert compared `heapUsed` to `heapTotal`, a heap utilisation ratio that says nothing about an impending OOM kill, and the iii-engine child process holding the state store was not measured at all (worker at 367 MB while the engine sat at 1120 MB). `/agentmemory/health` now reads the cgroup so it sees the whole container, warns at 75% and 90% of the limit, logs one line per status transition (the crashes previously left nothing in the log), and reports retrieval and embed queue depth, deadline-exceeded counts, the widest `kv.list` scopes, and per-operation latency percentiles for enrich, search, and file-context.
- **New `npm run bench:hook-burst` harness** reproduces the Cursor hook load shape — concurrent observe+enrich pairs with the real 2500 ms client abort against a production-shaped corpus — and reports latency, abort rate, and process RSS. It refuses to write to a non-loopback target unless `BURST_ALLOW_REMOTE=1`.
