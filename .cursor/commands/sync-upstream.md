...
| Graph search uses write-path indexes | `src/state/graph-search-index.ts`, `src/functions/graph-retrieval.ts` | When `AGENTMEMORY_GRAPH_SEARCH` is on, lookup goes through token / adjacency / obs-to-nodes indexes namespaced by `resetAt`. It must not fall back to listing the graph tables. |
