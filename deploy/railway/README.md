...
2. Set the service **Root Directory** to `/` (repository root). The
   Dockerfile builds from the checked-out source via `COPY . .`, so a
   root of `/deploy/railway` cannot see `package.json` / `src/`.
