...
- Idle catch-up (`mem::idle-sweep`) runs only when an LLM provider is configured. Default: every 15m, sessions idle ≥30m **or** ≥25 pending observations, max 5 sessions/sweep, 60m per-session cooldown. Disable with `AGENTMEMORY_IDLE_SWEEP_ENABLED=false`.
