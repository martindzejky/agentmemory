...
- **Pass E — first-class assistant and subagent events.** `assistant_response`, `subagent_start`, and `subagent_stop` are lifted and summarized like tool observations. Adapters no longer need to disguise conversation replies or subagent lifecycle as fake `post_tool_use` tool names.
