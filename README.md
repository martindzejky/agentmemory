...
- **Pass E — assistant and subagent compression.** `assistant_response`, `subagent_start`, and `subagent_stop` are lifted into the compression path (synthetic or LLM) so titles and narratives summarize them like tool observations. The lifted fields do not persist as a durable raw event log; compression overwrites the same observation key. Adapters can send these hook types directly instead of remapping them to fake `post_tool_use` tool names.
