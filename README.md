...
- **Pass C — idle catch-up sweep.** A bounded `mem::idle-sweep` timer processes sessions that gained observations, went idle, and have not been swept since. It reuses `event::session::stopped` with `skipConsolidation` (no per-session corpus consolidate/crystallize). Turn-boundary `POST /summarize` stays primary; eviction recovery is unchanged. Kill switch / interval / idle threshold / batch cap / per-session cooldown via `AGENTMEMORY_IDLE_SWEEP_*` and `AGENTMEMORY_IDLE_THRESHOLD_MS`.
