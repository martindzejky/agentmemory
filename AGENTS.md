...
`state::list` has no pagination: the engine materialises the whole scope as one JSON array and ships it over the WebSocket. Never call `kv.list` on an unbounded scope from a request path. `KV.graphNodes` / `KV.graphEdges` are the worst offenders (32K / 61K rows in one production deployment, 60.8 MB of JSON, ~2.1s, and ~570 MB of iii-engine RSS that is never released per call). Graph viewers and unexposed temporal query read the snapshot (`loadSnapshotGraph`). Hybrid graph search, when `AGENTMEMORY_GRAPH_SEARCH=true`, uses write-path token / adjacency / observation indexes namespaced by `resetAt` and never lists those scopes. The flag stays off by default. After a graph reset the raw tables still hold orphaned rows; do not index or vacuum them. Full context: `docs/investigations/2026-08-24-latency-and-oom.md`.
