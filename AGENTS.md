...
`state::list` has no pagination: the engine materialises the whole scope as one JSON array and ships it over the WebSocket. Never call `kv.list` on an unbounded scope from a request path. `KV.graphNodes` / `KV.graphEdges` are the worst offenders (32K / 61K rows in one production deployment, 60.8 MB of JSON, ~2.1s, and ~570 MB of iii-engine RSS that is never released per call), which is why the graph stream in hybrid search is behind `AGENTMEMORY_GRAPH_SEARCH` and off by default. After a graph reset those tables still hold orphaned rows; readers must use the snapshot (`loadSnapshotGraph`), never `kv.list` those scopes. Full context: `docs/investigations/2026-08-24-latency-and-oom.md`.
